    ...
    <form @submit.prevent="submitForm" class="form">
      <div class="form-group">
        ...
        <input type="text" id="first-name" v-model.trim="employee.editedEmployee.firstName"
          @input="checkMaxLength('firstName'); clearError('firstNameError')" required>
        <div class="error-message" v-if="firstNameError">{{ firstNameError }}</div>
      </div>
      <div class="form-group">
        <label for="last-name">Last Name:</label>
        <input type="text" id="last-name" v-model.trim="employee.editedEmployee.lastName" @input="checkMaxLength('lastName'); clearError('lastNameError')"
          required>
        <div class="error-message" v-if="lastNameError">{{ lastNameError }}</div>
      </div>
      <div class="form-group">
        <label for="email">Email:</label>
        <input type="email" id="email" v-model.trim="employee.editedEmployee.email" @input="clearError('emailError')" required>
        <div class="error-message" v-if="emailError">{{ emailError }}</div>
      </div>
      <div class="form-group">
        <label for="phone">Phone number:</label>
        <input type="tel" id="phone" v-model.trim="employee.editedEmployee.phone" @input="clearError('phoneError')" required>
      ...
      <div class="form-group">
        ...
        <input type="text" id="company" v-model.trim="employee.editedEmployee.companyName"
          @input="checkMaxLength('companyName'); clearError('companyNameError')" required>
...
export default {
  ...
  methods: {
    checkMaxLength(fieldName) {
      const maxNameLength = 20;

      const inputValue = this.employee.editedEmployee[fieldName];

      if (inputValue.length > maxNameLength) {
        this.employee.editedEmployee[fieldName] = inputValue.slice(0, maxNameLength);
      }
    },
    submitForm() {
      this.clearErrors();
      ...
      } else if (this.employee.editedEmployee.firstName.length > maxNameLength) {
        this.firstNameError = `First name should not exceed ${maxNameLength} characters.`;
      }
      ...
      } else if (this.employee.editedEmployee.lastName.length > maxNameLength) {
        this.lastNameError = `Last name should not exceed ${maxNameLength} characters.`;
      }

      if (!/^[A-Za-z]+$/.test(this.employee.editedEmployee.companyName)) {
        this.companyNameError = 'Company name should only contain letters and no special characters.';
      } else if (this.employee.editedEmployee.companyName.length > maxNameLength) {
        this.companyNameError = `Company name should not exceed ${maxNameLength} characters.`;
      }
